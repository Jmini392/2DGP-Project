...
class Gorgon:
    ...
    def attack_player(self, power = 10):
        self.state = 'attack'
        return BehaviorTree.SUCCESS
    ...
    def move_to(self, r = 0.5):
        self.state = 'walk'
        self.move_little_to(self.tx, self.y)
        if self.distance_less_than(self.tx, self.y, self.x, self.y, r):
            return BehaviorTree.SUCCESS
        else:
            return BehaviorTree.RUNNING

    def build_behavior_tree(self):
        c1 = Condition('Player near', self.if_player_near, 7)
        a1 = Action('Move to player', self.move_to_player, 2)
        follow_player = Sequence('Follow_player', c1, a1)
        a2 = Action('Attack', self.attack_player, self.power)
        follow_and_attack = Sequence('Follow_and_attack', follow_player, a2)
        a3 = Action('Random walk', self.get_random_location)
        a5 = Action('Move to', self.move_to)
        random_walk = Sequence('random_walk', a3, a5)
        root = Selector('Gorgon', follow_and_attack, random_walk)
